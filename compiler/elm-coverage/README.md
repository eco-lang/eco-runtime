...
To build and run `elm-coverage` from source instead of using the npm package:

1. Clone the repository:

       git clone https://github.com/zwilias/elm-coverage.git
       cd elm-coverage

2. Install dependencies (this also downloads the `elm-instrument` binary):

       npm install

3. Build the Elm analyzer:

       npm run make

4. Run locally using the bin script:

       ./bin/elm-coverage

   Or from another project directory, use the full path:

       /path/to/elm-coverage/bin/elm-coverage

   You can also link it globally for convenience:

       npm link

   After linking, `elm-coverage` will be available as a command.
