...
function addCoverage(info, coverage) {
    Object.keys(coverage).forEach(function(module) {
        var moduleCoverage = coverage[module];

        if (Array.isArray(moduleCoverage)) {
            // Legacy array format: [idx, idx, idx, ...]
            moduleCoverage.forEach(function(idx) {
                info[module][idx].count = info[module][idx].count + 1 || 1;
            });
        } else {
            // Count-based format: { idx: count, idx: count, ... }
            Object.keys(moduleCoverage).forEach(function(idx) {
                var i = parseInt(idx, 10);
                info[module][i].count = (info[module][i].count || 0) + moduleCoverage[idx];
            });
        }
    });
