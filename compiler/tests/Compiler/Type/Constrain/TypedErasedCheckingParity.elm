...
This test verifies that when type checking fails, both the standard path and
the WithIds path fail for the same reasons:

  - Both paths must produce type errors (not succeed)
  - Both paths should produce the same number of errors
  - The errors should be equivalent (same error type at the same region)

-}
expectEquivalentTypeCheckingFails : Src.Module -> Expect.Expectation
expectEquivalentTypeCheckingFails srcModule =
    ...
    case Result.run result of
        ...
        ( _, Ok modul ) ->
            let
                standardResult =
                    IO.unsafePerformIO (runStandardPathWithErrors modul)

                withIdsResult =
                    IO.unsafePerformIO (runWithIdsPathWithErrors modul)
            in
            case ( standardResult, withIdsResult ) of
                ( Ok _, Ok _ ) ->
                    -- Both succeeded - but we expected failure!
                    Expect.fail "Expected type checking to fail, but both paths succeeded."

                ( Err standardErrors, Err withIdsErrors ) ->
                    -- Both failed - verify they failed for equivalent reasons
                    let
                        standardErrorList =
                            NE.toList standardErrors
...
{-| Run the standard constraint generation and solving path, returning actual errors.
-}
...
{-| Run the WithIds constraint generation and solving path, returning actual errors.
-}
