 ...
 * eco-boot-runner.js
 *
 * Bootstrap entry point for running the Eco compiler under Node.js with
 * XHR-based IO. This script:
 *
 * 1. Sets up a mock XMLHttpRequest environment (same as index.js)
 * 2. Registers the eco-io handler for Eco.* IO operations
 * 3. Registers legacy handlers for compiler operations not yet in Eco.*
 * 4. Loads and runs the bootstrap-compiled Eco compiler (eco-boot.js)
 *
 * Usage:
 *   node eco-boot-runner.js [compiler args...]
 */
...
// --- Archive download helper (same as index.js) ---
...
// =================================================================
// ECO-IO HANDLER: Routes new-style JSON requests to eco-io-handler
// =================================================================
...
// =================================================================
// LEGACY HANDLERS: Same as index.js for operations not yet in Eco.*
// These support the compiler's Utils/Main.elm and System/IO.elm
// operations that go through Utils/Impure.elm directly.
// =================================================================
...
server.post("httpUpload", (request) => {
  ...
  form.pipe(req);
  req.on("response", (res) => {
    res.on("end", () => request.respond(200));
  });
  req.on("error", (err) => {
    throw err;
  });
...
server.post("withCreateProcess", (request) => {
  ...
  tmp.file((err, path, fd) => {
    ...
    fs.createReadStream(path)
      .on("data", (chunk) => processes[nextCounter].stdin.write(chunk))
      .on("close", () => processes[nextCounter].stdin.end());
...
server.post("dirFindExecutable", (request) => {
  const p = which.sync(request.body, { nothrow: true }) ?? null;
  request.respond(200, null, JSON.stringify(p));
});
...
// Default handler for HTTP proxying
