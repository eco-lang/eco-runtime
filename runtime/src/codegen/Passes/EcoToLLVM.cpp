...
/// Get or insert a function declaration for a runtime function.
...
/// Convert HPointer constant kind to the encoded i64 value.
/// The constant field is at bits 40-43 of the HPointer.
/// Note: The ConstantKind enum values are already 1-based (Unit=1, ..., EmptyString=7)
/// matching the HPointer encoding where 0 means regular pointer.
...
struct ConstantOpLowering : public OpConversionPattern<ConstantOp> {
    ...
    matchAndRewrite(ConstantOp op, OpAdaptor adaptor,
        ...
        // Get the constant kind and encode it
...
struct DbgOpLowering : public OpConversionPattern<DbgOp> {
    ...
    matchAndRewrite(DbgOp op, OpAdaptor adaptor,
        ...
        // Allocate array for arguments on stack
...
struct UnboxOpLowering : public OpConversionPattern<UnboxOp> {
    ...
    matchAndRewrite(UnboxOp op, OpAdaptor adaptor,
        ...
        // Offset 8 bytes (skip Header) to get to the value field
...
struct ConstructOpLowering : public OpConversionPattern<ConstructOp> {
    ...
    matchAndRewrite(ConstructOp op, OpAdaptor adaptor,
        ...
        // Convert ptr to i64 for the result
...
struct ProjectOpLowering : public OpConversionPattern<ProjectOp> {
    ...
    matchAndRewrite(ProjectOp op, OpAdaptor adaptor,
        ...
        // Calculate offset based on Custom layout:
        // Header (8) + ctor/unboxed (8) + index * 8
...
struct StringLiteralOpLowering : public OpConversionPattern<StringLiteralOp> {
    ...
    matchAndRewrite(StringLiteralOp op, OpAdaptor adaptor,
        ...
        // Handle empty string -> use embedded constant
        if (utf8Value.empty()) {
        ...
        // Build the header: tag=Tag_String(3), size=utf16.size()
        // Header is 64 bits: tag(5) | color(2) | pin(1) | epoch(2) | age(2) | unboxed(3) | padding(1) | refcount(16) | size(32)
        ...
        // For LLVM globals, we need to use LLVM::GlobalOp
        // The type is the struct type
        ...
        // Get address of global and convert to i64
...
struct CallOpLowering : public OpConversionPattern<CallOp> {
    ...
    matchAndRewrite(CallOp op, OpAdaptor adaptor,
        ...
        if (callee) {
            ...
            // Set tailcc for Eco-to-Eco calls
