...
- **Node.js**: v22+ works (tested with v22.21.0)
- **npm**: v10+ works (tested with 10.9.4)
...
Build outputs:
- **API build**: `lib/guida.js` (~3.8MB) → `lib/guida.min.js` (~752KB) → ~200KB gzipped
- **CLI build**: `bin/guida.js` (~4.2MB) → `bin/guida.min.js` (~860KB) → ~232KB gzipped

The compiler compiles 150 modules for the API and 18 modules for the terminal interface.

## Running the Compiler

From the `/work/compiler` directory:
...
- **lib/guida.js** / **lib/guida.min.js** - Built API (generated by `npm run build:api`)
- **lib/guida.browser.js** / **lib/guida.browser.min.js** - Browser-specific build (pre-built)
- **lib/guida.node.js** / **lib/guida.node.min.js** - Node-specific build (pre-built)

### Binary Entry Point (`bin/`)

- **bin/index.js** - CLI entry point wrapper
- **bin/guida.js** / **bin/guida.min.js** - Built CLI (generated by `npm run build:bin`)
...
After building, verify the compiler works:

```bash
cd /work/compiler
node bin/index.js --version    # Should output: 1.0.0
node bin/index.js --help       # Should display help text
```
