...
module {
  func.func @main() -> i64 {
    ...
    %ctor0 = eco.construct.custom() {tag = 0 : i64, size = 0 : i64} : () -> !eco.value
    eco.dbg %ctor0 : !eco.value
    // CHECK: Ctor0

    // Tag 1 with no fields (like Nothing in Maybe)
    %ctor1 = eco.construct.custom() {tag = 1 : i64, size = 0 : i64} : () -> !eco.value
    eco.dbg %ctor1 : !eco.value
    // CHECK: Ctor1

    // Tag 2 with no fields
    %ctor2 = eco.construct.custom() {tag = 2 : i64, size = 0 : i64} : () -> !eco.value
    eco.dbg %ctor2 : !eco.value
    // CHECK: Ctor2

    // Tag 10 with no fields
    %ctor10 = eco.construct.custom() {tag = 10 : i64, size = 0 : i64} : () -> !eco.value
    eco.dbg %ctor10 : !eco.value
    // CHECK: Ctor10

    // Large tag with no fields
    %ctor100 = eco.construct.custom() {tag = 100 : i64, size = 0 : i64} : () -> !eco.value
