...
module {
  func.func @main() -> i64 {
    ...
    %mixed1 = eco.construct(%b10, %i20, %b30, %i40) {tag = 1 : i64, size = 4 : i64, unboxed_bitmap = 10 : i64} : (!eco.value, i64, !eco.value, i64) -> !eco.value
    eco.dbg %mixed1 : !eco.value
    // CHECK: Ctor1 10 20 30 40

    // Project boxed field (index 0)
    %p0 = eco.project %mixed1[0] : !eco.value -> !eco.value
    eco.dbg %p0 : !eco.value
    // CHECK: 10

    // Project unboxed field (index 1)
    %p1 = eco.project %mixed1[1] : !eco.value -> i64
    eco.dbg %p1 : i64
    // CHECK: 20

    // Project boxed field (index 2)
    %p2 = eco.project %mixed1[2] : !eco.value -> !eco.value
    eco.dbg %p2 : !eco.value
    // CHECK: 30

    // Project unboxed field (index 3)
    %p3 = eco.project %mixed1[3] : !eco.value -> i64
    eco.dbg %p3 : i64
    // CHECK: 40
    ...
    %mixed2 = eco.construct(%i10, %b20, %i30, %b40) {tag = 2 : i64, size = 4 : i64, unboxed_bitmap = 5 : i64} : (i64, !eco.value, i64, !eco.value) -> !eco.value
    eco.dbg %mixed2 : !eco.value
    // CHECK: Ctor2 10 20 30 40

    // Project each field
    %q0 = eco.project %mixed2[0] : !eco.value -> i64
    eco.dbg %q0 : i64
    // CHECK: 10

    %q1 = eco.project %mixed2[1] : !eco.value -> !eco.value
    eco.dbg %q1 : !eco.value
    // CHECK: 20

    %q2 = eco.project %mixed2[2] : !eco.value -> i64
    eco.dbg %q2 : i64
    // CHECK: 30

    %q3 = eco.project %mixed2[3] : !eco.value -> !eco.value
    eco.dbg %q3 : !eco.value
    // CHECK: 40

    // Pattern: [boxed, boxed, unboxed, unboxed]
    // unboxed_bitmap = 12 (0b1100) - bits 2 and 3 are unboxed
    %mixed3 = eco.construct(%b10, %b20, %i30, %i40) {tag = 3 : i64, size = 4 : i64, unboxed_bitmap = 12 : i64} : (!eco.value, !eco.value, i64, i64) -> !eco.value
    eco.dbg %mixed3 : !eco.value
    // CHECK: Ctor3 10 20 30 40

    // Project unboxed fields
    %r2 = eco.project %mixed3[2] : !eco.value -> i64
    eco.dbg %r2 : i64
    // CHECK: 30

    %r3 = eco.project %mixed3[3] : !eco.value -> i64
    eco.dbg %r3 : i64
    // CHECK: 40
