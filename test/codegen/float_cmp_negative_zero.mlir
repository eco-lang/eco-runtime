...
module {
  func.func @main() -> i64 {
    ...
    %eq1 = eco.float.cmp eq %neg_zero, %pos_zero : f64
    %eq1_ext = arith.extui %eq1 : i1 to i64
    eco.dbg %eq1_ext : i64
    // CHECK: 1

    // Test: +0.0 == -0.0 should be true (commutative)
    %eq2 = eco.float.cmp eq %pos_zero, %neg_zero : f64
    %eq2_ext = arith.extui %eq2 : i1 to i64
    eco.dbg %eq2_ext : i64
    // CHECK: 1

    // Test: -0.0 != +0.0 should be false
    %ne1 = eco.float.cmp ne %neg_zero, %pos_zero : f64
    %ne1_ext = arith.extui %ne1 : i1 to i64
    eco.dbg %ne1_ext : i64
    // CHECK: 0

    // Test: -0.0 < +0.0 should be false
    %lt1 = eco.float.cmp lt %neg_zero, %pos_zero : f64
    %lt1_ext = arith.extui %lt1 : i1 to i64
    eco.dbg %lt1_ext : i64
    // CHECK: 0

    // Test: +0.0 < -0.0 should be false
    %lt2 = eco.float.cmp lt %pos_zero, %neg_zero : f64
    %lt2_ext = arith.extui %lt2 : i1 to i64
    eco.dbg %lt2_ext : i64
    // CHECK: 0

    // Test: -0.0 <= +0.0 should be true
    %le1 = eco.float.cmp le %neg_zero, %pos_zero : f64
    %le1_ext = arith.extui %le1 : i1 to i64
    eco.dbg %le1_ext : i64
    // CHECK: 1

    // Test: -0.0 >= +0.0 should be true
    %ge1 = eco.float.cmp ge %neg_zero, %pos_zero : f64
    ...
    %min_eq_zero = eco.float.cmp eq %min_zeros, %pos_zero : f64
    %max_eq_zero = eco.float.cmp eq %max_zeros, %pos_zero : f64
