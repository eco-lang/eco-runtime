...
module {
  func.func @main() -> i64 {
    ...
    %value = eco.construct(%b42) {tag = 0 : i64, size = 1 : i64} : (!eco.value) -> !eco.value

    // Level 3: Some(Value 42) (tag=1, one field)
    %some = eco.construct(%value) {tag = 1 : i64, size = 1 : i64} : (!eco.value) -> !eco.value

    // Level 2: Left(Some(Value 42)) (tag=0, one field)
    %left = eco.construct(%some) {tag = 0 : i64, size = 1 : i64} : (!eco.value) -> !eco.value

    // Level 1: Just(Left(Some(Value 42))) (tag=1, one field)
    %just = eco.construct(%left) {tag = 1 : i64, size = 1 : i64} : (!eco.value) -> !eco.value
    ...
    %inner1 = eco.project %just[0] : !eco.value -> !eco.value
    eco.case %inner1 [0, 1] {
    ...
    %inner3 = eco.project %inner2[0] : !eco.value -> !eco.value
    %final = eco.project %inner3[0] : !eco.value -> !eco.value
    eco.dbg %final : !eco.value
    // CHECK: 42

    // Now test a different path: Right
    %right = eco.construct(%some) {tag = 1 : i64, size = 1 : i64} : (!eco.value) -> !eco.value
    %just2 = eco.construct(%right) {tag = 1 : i64, size = 1 : i64} : (!eco.value) -> !eco.value
    ...
    %inner_r = eco.project %just2[0] : !eco.value -> !eco.value
