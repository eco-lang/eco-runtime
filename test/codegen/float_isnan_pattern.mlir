...
// Note: eco.float.{lt,le,gt,ge,eq,ne} uses ordered comparisons, so NaN != NaN returns false.
...
module {
  func.func @main() -> i64 {
    ...
    %one_eq_one = eco.float.eq %one, %one : f64
    %one_eq_ext = arith.extui %one_eq_one : i1 to i64
    eco.dbg %one_eq_ext : i64
    // CHECK: 1

    %one_ne_one = eco.float.ne %one, %one : f64
    ...
    %inf_eq_inf = eco.float.eq %inf, %inf : f64
    %inf_eq_ext = arith.extui %inf_eq_inf : i1 to i64
    eco.dbg %inf_eq_ext : i64
    // CHECK: 1

    %inf_ne_inf = eco.float.ne %inf, %inf : f64
