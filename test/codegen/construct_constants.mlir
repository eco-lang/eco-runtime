...
//   %bad = eco.construct(%nil) {unboxed_bitmap = 1} : (!eco.value) -> !eco.value
...
module {
  func.func @main() -> i64 {
    ...
    %c1 = eco.construct(%nil) {tag = 1 : i64, size = 1 : i64} : (!eco.value) -> !eco.value
    eco.dbg %c1 : !eco.value
    // CHECK: Ctor1 []

    %c2 = eco.construct(%true) {tag = 2 : i64, size = 1 : i64} : (!eco.value) -> !eco.value
    eco.dbg %c2 : !eco.value
    // CHECK: Ctor2 True

    %c3 = eco.construct(%false) {tag = 3 : i64, size = 1 : i64} : (!eco.value) -> !eco.value
    eco.dbg %c3 : !eco.value
    // CHECK: Ctor3 False

    %c4 = eco.construct(%unit) {tag = 4 : i64, size = 1 : i64} : (!eco.value) -> !eco.value
    eco.dbg %c4 : !eco.value
    // CHECK: Ctor4 ()

    %c5 = eco.construct(%nothing) {tag = 5 : i64, size = 1 : i64} : (!eco.value) -> !eco.value
    eco.dbg %c5 : !eco.value
    // CHECK: Ctor5 Nothing

    // === Multiple constant fields ===
    %pair_consts = eco.construct(%nil, %true) {tag = 10 : i64, size = 2 : i64} : (!eco.value, !eco.value) -> !eco.value
    eco.dbg %pair_consts : !eco.value
    // CHECK: Ctor10 [] True

    %triple_consts = eco.construct(%true, %false, %nil) {tag = 11 : i64, size = 3 : i64} : (!eco.value, !eco.value, !eco.value) -> !eco.value
    eco.dbg %triple_consts : !eco.value
    // CHECK: Ctor11 True False []

    // === Project constants from structure ===
    %proj0 = eco.project %pair_consts[0] : !eco.value -> !eco.value
    eco.dbg %proj0 : !eco.value
    // CHECK: []

    %proj1 = eco.project %pair_consts[1] : !eco.value -> !eco.value
    ...
    %mixed1 = eco.construct(%b42, %nil) {tag = 20 : i64, size = 2 : i64} : (!eco.value, !eco.value) -> !eco.value
    eco.dbg %mixed1 : !eco.value
    // CHECK: Ctor20 42 []

    %mixed2 = eco.construct(%true, %b99) {tag = 21 : i64, size = 2 : i64} : (!eco.value, !eco.value) -> !eco.value
    eco.dbg %mixed2 : !eco.value
    // CHECK: Ctor21 True 99

    %mixed3 = eco.construct(%b42, %false, %b99) {tag = 22 : i64, size = 3 : i64} : (!eco.value, !eco.value, !eco.value) -> !eco.value
    ...
    %mix_unboxed1 = eco.construct(%i10, %nil) {tag = 30 : i64, size = 2 : i64, unboxed_bitmap = 1 : i64} : (i64, !eco.value) -> !eco.value
    eco.dbg %mix_unboxed1 : !eco.value
    // CHECK: Ctor30 10 []

    // unboxed_bitmap = 2 means field 1 is unboxed
    %i20 = arith.constant 20 : i64
    %mix_unboxed2 = eco.construct(%true, %i20) {tag = 31 : i64, size = 2 : i64, unboxed_bitmap = 2 : i64} : (!eco.value, i64) -> !eco.value
    ...
    %mix_unboxed3 = eco.construct(%i10, %false, %i30) {tag = 32 : i64, size = 3 : i64, unboxed_bitmap = 5 : i64} : (i64, !eco.value, i64) -> !eco.value
    eco.dbg %mix_unboxed3 : !eco.value
    // CHECK: Ctor32 10 False 30

    // Project fields to verify values
    %p_unboxed = eco.project %mix_unboxed3[0] : !eco.value -> i64
    eco.dbg %p_unboxed : i64
    // CHECK: 10

    %p_const = eco.project %mix_unboxed3[1] : !eco.value -> !eco.value
    eco.dbg %p_const : !eco.value
    // CHECK: False

    // === List with Nil tail (Cons with constant) ===
    // This is the most common case: Cons cell with Nil as tail
    %list1 = eco.construct(%b42, %nil) {tag = 0 : i64, size = 2 : i64} : (!eco.value, !eco.value) -> !eco.value
