...
module {
  func.func @main() -> i64 {
    ...
    %just42 = eco.construct(%b42) {tag = 0 : i64, size = 1 : i64} : (!eco.value) -> !eco.value
    eco.dbg %just42 : !eco.value
    // CHECK: Ctor0 42

    // Extract value from Just
    %just_val = eco.project %just42[0] : !eco.value -> !eco.value
    ...
    %ok_val = eco.construct(%b100) {tag = 0 : i64, size = 1 : i64} : (!eco.value) -> !eco.value
    ...
    %err_val = eco.construct(%b999) {tag = 1 : i64, size = 1 : i64} : (!eco.value) -> !eco.value
    eco.dbg %err_val : !eco.value
    // CHECK: Ctor1 999

    // Extract from Err
    %err_inner = eco.project %err_val[0] : !eco.value -> !eco.value
    ...
    %inner_just = eco.construct(%b7) {tag = 0 : i64, size = 1 : i64} : (!eco.value) -> !eco.value
    %outer_just = eco.construct(%inner_just) {tag = 0 : i64, size = 1 : i64} : (!eco.value) -> !eco.value
    eco.dbg %outer_just : !eco.value
    // CHECK: Ctor0 (Ctor0 7)

    // Unwrap twice
    %level1 = eco.project %outer_just[0] : !eco.value -> !eco.value
    %level2 = eco.project %level1[0] : !eco.value -> !eco.value
    ...
    %red = eco.construct(%dummy) {tag = 0 : i64, size = 1 : i64} : (!eco.value) -> !eco.value
    %green = eco.construct(%dummy) {tag = 1 : i64, size = 1 : i64} : (!eco.value) -> !eco.value
    %blue = eco.construct(%dummy) {tag = 2 : i64, size = 1 : i64} : (!eco.value) -> !eco.value
