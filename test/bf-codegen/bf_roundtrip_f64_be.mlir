// RUN: %ecoc %s -emit=jit 2>&1 | %FileCheck %s
//
// Test roundtrip: write f64 BE, read back, verify match

module {
  func.func @main() -> i64 {
    %val = arith.constant 3.141592653589793 : f64

    // Create buffer and write
    %size = arith.constant 16 : i32
    %buffer = bf.alloc %size : i64
    %cursor0 = bf.cursor.init %buffer : i64 -> !bf.cursor
    %cursor1 = bf.write.f64 %cursor0, %val (be) : !bf.cursor

    // Read back
    %read_cursor0 = bf.decoder.cursor.init %buffer : i64 -> !bf.cursor
    %result, %read_cursor1 = bf.read.f64 %read_cursor0 (be) : f64, !bf.cursor

    // Verify match
    %match = arith.cmpf oeq, %val, %result : f64
    %match_int = arith.extui %match : i1 to i64
    eco.dbg %match_int : i64
    // CHECK: 1

    %zero = arith.constant 0 : i64
    return %zero : i64
  }
}
